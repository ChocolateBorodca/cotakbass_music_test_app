...
def registration_screen():
    st.markdown("""
        ...
        /* Растягиваем невидимый загрузчик на 100% площади */
        ...
        .center-btn button { background: rgba(255, 255, 255, 0.02) !important; border: 1px solid rgba(160, 32, 240, 0.4) !important; border-radius: 50% !important; color: #A020F0 !important; width: 65px !important; height: 65px !important; font-size: 28px !important; }
        </style>
    """, unsafe_allow_html=True)

    st.markdown('<div style="text-align:center; color:#A020F0; letter-spacing:5px; margin-top:20px; font-size:10px;">REGISTRATION</div>', unsafe_allow_html=True)
    
    # ФОН
    st.markdown('<div class="upload-area" style="max-width:500px; height:160px;"><div class="bg-draw">+</div>', unsafe_allow_html=True)
    st.file_uploader("bg", key="reg_bg")
    st.markdown('</div>', unsafe_allow_html=True)
    
    # АВА
    st.markdown('<div class="upload-area" style="width:130px; height:130px; margin-top:-65px;"><div class="ava-draw">', unsafe_allow_html=True)
    u_ava = st.file_uploader("ava", key="reg_ava")
    img_64 = get_base64(u_ava)
