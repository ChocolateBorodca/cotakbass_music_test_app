...
# Пытаемся импортировать регистрацию. Если папки нет — выведем ошибку.
try:
    from components.auth import registration_screen
except ImportError:
    st.error("Ошибка: Создай на GitHub папку 'components' и положи туда файл 'auth.py'!")
    st.stop()
...
if not st.session_state.auth:
    # ПОКАЗЫВАЕМ ТОЛЬКО РЕГИСТРАЦИЮ
    registration_screen()
    
    # Кнопка для теста (если хочешь сбросить сессию и увидеть экран снова)
    if st.sidebar.button("Сбросить вход"):
        st.session_state.auth = False
        st.rerun()
else:
    # ТВОЙ ИДЕАЛЬНЫЙ ПЛЕЕР (НЕ ТРОГАЕМ)
