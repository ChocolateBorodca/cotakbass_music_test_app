...
# Ультра-минимализм Apple + МОБИЛЬНАЯ АДАПТАЦИЯ
st.markdown(f"""
    <style>
    html, body, [class*="st-"] {{
    ...
    /* Адаптация заголовка и текста для мобилок */
    .app-header {{
    ...
    .track-title {{ 
        font-size: clamp(24px, 8vw, 48px); /* Шрифт подстраивается под экран */
    ...
    /* Контейнер для кнопок, чтобы они не разлетались на мобиле */
    .button-group {{
        display: flex;
    ...
    /* Убираем эффект ховера для мобилок (чтобы не залипало) и оставляем только нажатие */
    @media (hover: hover) {{
...
if not tracks:
    ...
    if st.session_state.page == "library":
        ...
        # Используем 4 колонки с фиксированным небольшим отступом для мобильных
