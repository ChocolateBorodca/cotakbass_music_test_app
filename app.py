...
import random
import base64
from components.auth import registration_screen # Подключаем новый файл регистрации

# Настройки (из твоего Файла 1)
...
# Session State (из твоего Файла 1)
...
if not st.session_state.auth:
    registration_screen() # Показываем только твой рисунок
else:
    # --- ВЕСЬ ТВОЙ КОД ИЗ 3-Х ФАЙЛОВ (ПЛЕЕР) ---
    ...
    st.markdown(f"""
        ...
        div.stButton > button {{ background: rgba(255, 255, 255, 0.02) !important; backdrop-filter: blur(30px) !important; border: 1px solid rgba(255, 255, 255, 0.1) !important; border-radius: 50% !important; color: #A020F0 !important; width: 52px !important; height: 52px !important; transition: 0.35s ease; }}
    ...
    if tracks:
        # Логика экранов Search / Library (Файл 2)
